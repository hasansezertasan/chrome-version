...
- Provide project and platform versions (Python, `uv`, `tox`, OS/architecture, and the `chrome-version` version), depending on what seems relevant.
...
- **Include the exact command and its full terminal output** (copy the text, not a screenshot) so the behavior you are describing is unambiguous.
